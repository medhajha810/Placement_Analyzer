...
async function seedDatabase() {
  ...
  try {
    ...
    // 1. Insert companies (minimal fields)
    console.log("📦 Inserting companies...");
    const minimalCompanies = companies.map(c => ({ name: c.name }));
    
    const { data: insertedCompanies, error: companiesError } = await supabase
      .from("companies")
      .upsert(minimalCompanies, { onConflict: "name" })
      .select();
    ...
    const drivesToInsert = placementDrives.map((drive) => ({
      ...
      status: "active",
      drive_date: new Date(Date.now() + 25 * 24 * 60 * 60 * 1000).toISOString()
    }));
